# Sakura (Illium personal adaptation)
...
Illium shares its accent between focused borders, active workspaces, launcher selection and the generic WezTerm cursor, and its overlay between selection and unfocused borders; these roles are not one-to-one with Omarchy's application-specific styles. This is a native Illium palette, not an installation of Omarchy configuration. The upstream `icons.theme` (`Yaru-red`) has no Illium equivalent and is ignored. No upstream code is executed.

## Preview and wallpapers

- `preview.png`: the upstream root `preview.png`, copied byte-for-byte (3841 x 2161). It is an Omarchy desktop screenshot supplied by the author, not a live rendering of Illium windows.
