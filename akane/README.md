# Akane — Illium (personal adaptation)
...
The palette comes from upstream `colors.toml`. Illium's surface uses `lighter_background`, overlay uses `selection`, and secondary text uses `dark_foreground` rather than the darker `muted`. All 16 Alacritty colors are preserved exactly.
...
Install: `illiumctl theme install <path-to-this-folder>`, then `illiumctl theme set akane`.
