...
Collection locale indépendante du dépôt Illium. Chaque dossier est un pack installable : palette Illium, wallpapers, attribution et provenance des sources.
...
Avec une version d'Illium prenant en charge `theme install` et
`background_opacity` (mettre à jour tous les exécutables avant installation) :

```powershell
illiumctl theme install "C:\Downloads\pissarro"
illiumctl theme set pissarro
```

Sur cette machine, les sources sont aussi accessibles depuis Windows :

```powershell
illiumctl theme install "\\wsl.localhost\Debian\home\yannick\dev\illium-themes\pissarro"
```

Les onze packs sont déjà installés sous `%USERPROFILE%\.config\illium\themes`. Une nouvelle installation du même nom est volontairement refusée pour préserver les réglages existants.
...
Illium rendue explicite. Elle règle le fond du terminal, de Tasks et de Files,
ainsi que l'accueil du browser ; les pages web restent opaques. C'est un choix
Illium, pas une nouvelle couleur ni une valeur attribuée aux sources Omarchy.
L'ancien nom `terminal_background_opacity` reste accepté par Illium, mais les
...
ou redémarrer Illium rétablit leur valeur. Ajouter les deux raccourcis à un
...
- `illiumctl wallpaper next`, `wallpaper set "nom du fichier.jpg"`, `wallpaper clear` : commandes équivalentes.
...
Chaque pack inclut désormais `preview.png`, récupéré sans recompression depuis la même révision amont que sa palette ou depuis l'image référencée par son README figé. Pour Synthwave84, la capture est une pièce jointe GitHub externe : sa propre somme SHA-256 fixe son contenu, sans prétendre qu'elle est un fichier du commit Git. Le nouveau sélecteur utilise ce fichier plutôt qu'un wallpaper de secours. Ces visuels représentent les thèmes Omarchy, pas les applications Illium. Le renommage Dracula et les sommes SHA-256 sont documentés dans les `SOURCES.md`.
...
Copier l'un des dossiers, lui donner un identifiant en minuscules (`a-z`, `0-9`, `-`, `_`), puis modifier `theme.toml` et remplacer les fichiers de `wallpapers/`. Mettre à jour l'attribution et les licences : ne pas conserver celles d'un thème sans rapport. Installer le nouveau dossier avec `illiumctl theme install`.
...
## Skill de conversion Omarchy → Illium

Le skill versionné dans [`skills/omarchy-to-illium`](skills/omarchy-to-illium/SKILL.md) guide la conversion d'une URL GitHub ou d'un dossier local : révision source figée, palette adaptée, previews et wallpapers préservés, attribution, validation et installation sans toucher au dépôt Illium. Le guide [assets](skills/omarchy-to-illium/references/assets.md) couvre aussi les captures liées dans les README, les pièces jointes GitHub, la conversion des formats et l'ajout sans écrasement à un pack existant.

Il est installé sur cette machine via un lien dans `~/.agents/skills/omarchy-to-illium`. Dans pi, exécuter `/reload` pour le découvrir dans une session déjà ouverte, puis :

```text
/skill:omarchy-to-illium https://github.com/bjarneo/omarchy-snow-theme
```

On peut aussi demander naturellement « ajoute ce thème Omarchy à Illium » avec l'URL. Le skill ne change pas le thème actif sans demande explicite et ne publie pas les ressources tierces.
...
```bash
python3 -B -m unittest discover -s skills/omarchy-to-illium/tests -v
```
